with t_frequencies, Ada.Text_IO, Ada.Float_Text_IO;
use t_frequencies, Ada.Text_IO, Ada.Float_Text_IO;
package body t_frequencies is
	
	procedure tbuida (tfreq: out abecedari) is
	begin
		tfreq:= (others => 0.0);
	end tbuida;
		
	procedure mostra (tfreq: in abecedari) is 
	begin
		for c in indexabc'range loop
			new_line;
			put(c); put(":  ");
			put(tfreq(c),1,2,0);
		end loop;
	end mostra;
	
	--Omple la taula de frequecies en funció de l'entrada
	procedure omple (tfreq: out abecedari; fname: in String; ok: out boolean) is
		F_Entrada: File_Type;
		c: character;
		n: float;
	begin
		Open (F_Entrada, Mode => In_File, Name => fname); 
		
		ok:= is_open (F_Entrada);
		
		tbuida(tfreq);
		n:=0.0;
		while not End_Of_File (F_Entrada) loop
			n:= n+1.0;
			get (F_Entrada, c);
			if c in indexabc'range then
				tfreq(c):= tfreq(c)+1.0;
			end if;
		end loop;
		
		for c in indexabc loop
			tfreq(c):= (tfreq(c)/n);
		end loop;
		Close(F_Entrada);
	end omple;
	
	--iterador
	procedure primer (ce: in abecedari; it: out iterador) is
		i: indexabc renames it.i;
		valid: boolean renames it.valid;		
	begin
		i:= indexabc'First;
		while ce(i)=0.0 and i < indexabc'Last loop
			i:= indexabc'succ(i);
		end loop;
		if ce(i)=0.0 then
			valid:= false;
		else
			valid:= true;
		end if;
	end primer;
	
	procedure succesor (ce: in abecedari; it: in out iterador)is
		i: indexabc renames it.i;
		valid: boolean renames it.valid;
	begin
		if not valid then raise bad_it; end if;
		if i<indexabc'last then
			i:= indexabc'succ(i);
			while ce(i)=0.0 and i<indexabc'last loop
				i:= indexabc'succ(i);
			end loop;
			if  ce(i)=0.0 then 
				valid:= false;
			else
				valid:= true;
			end if;
		else
			valid:= false;
		end if;
	end succesor;
	
	procedure consulta (ce: in abecedari; it: in iterador; c: out indexabc; v: out float) is
		i: indexabc renames it.i;
		valid: boolean renames it.valid;
	begin 
		if not valid then raise bad_it; end if;
		c:= i;
		v:= ce(i);
	end consulta; 
	
	function esValid (it: in iterador) return boolean is
	begin
		return it.valid;
	end esValid;

end t_frequencies;
	
